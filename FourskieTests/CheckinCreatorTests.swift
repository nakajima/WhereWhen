...
final class CheckinCreatorTests: XCTestCase {
	...
	func testWorks() async throws {
		...
		try await CheckinCreator(checkin: checkin, database: database).create()
	...
	func testDoesNotCreateIfCloseToIgnoredPlace() async throws {
		...
		try await CheckinCreator(checkin: newCheckin, database: database).create()
